...
class LeastSqRes(object):
    ...
    def __init__(self, NYxi, Ypred, PsiY, Nuxi, upred, Psiu, problem, sigma_r, sigma_p, sigma_q, res_fac, L, iuobs, uobs, iYobs, Yobs, beta, ssv=None):
        self.NYxi     = NYxi
        self.Nuxi     = Nuxi
    ...
    def val(self, x):
        ...
        if self.ssv is None:
            return np.concatenate((1/self.sigma_r *self.problem.residual(u, Y) / self.res_fac, 1/self.sigma_q * (self.L @ u), 1/self.sigma_p * (self.L @ Y)))
        else:
            return np.concatenate((1/self.sigma_r *self.problem.residual(u, Y)[self.ssv] / self.res_fac, 1/self.sigma_q * (self.L @ u), 1/self.sigma_p * (self.L @ Y)))
