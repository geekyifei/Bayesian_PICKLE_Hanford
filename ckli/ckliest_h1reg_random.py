...
class LeastSqRes(object):
    ...
    def __init__(self, NYxi, Ypred, PsiY, Nuxi, upred, Psiu, problem, sigma_r, sigma_p, sigma_q, res_fac, L, iuobs, uobs, iYobs, Yobs, beta,  a, b, w, ssv=None):
        self.NYxi     = NYxi
        self.Nuxi     = Nuxi
        ...
        self.Yobs     = Yobs
        self.L        = L
        self.jconst   = np.block([[1/self.sigma_q * (self.L @ self.Psiu), np.zeros((self.L.shape[0], self.NYxi))],
                                  [np.zeros((self.L.shape[0], self.Nuxi)), 1/self.sigma_p * (self.L @ self.PsiY)]])
        self.a = a
        self.b = b
        self.w = w
    ...
    def val(self, x):
        ...
        if self.ssv is None:
            return np.concatenate((1/self.sigma_r* (self.problem.residual(u, Y) - self.w)/ self.res_fac, 1/self.sigma_q * (self.L @ u - self.b), 1/self.sigma_p * (self.L @ Y - self.a)))
        else:
            return np.concatenate((1/self.sigma_r* self.problem.residual(u, Y)[self.ssv] / self.res_fac, 1/self.sigma_q * uxi, 1/self.sigma_p * Yxi))
